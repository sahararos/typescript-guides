...
let character = { // We have an object called character. Remember, objects are in curly braces. 
...
for (let x in character) { // The variable will hold the property names.
...
for (let x in character) {
    console.log(`${x} : ${character[x]}`); // We use bracket notation to access the property values.`);
} 
...
for (let i in salaries) {
    let salary = `$${salaries[i]}`;// In template literals we're going to say $ and then pull in our property values by saying {salaries[i]}'. In other words, we're adding a dollar sign in front of the salaries and storing that inside the variable "salaries".
