...
for (let i = 0; i <= 10; i++) { // Remember, the expression i++ is a shorthand way of writing i = i + 1.
...
for (let i = 0; i < characters.length; i++ ) { // The condition: i less than the number of values in the array (I have 5 values/characters).
    if (i === characters.length - 1) {
        info += `and ${characters[i]}.`; // The "characters[i]" part is how we select a value inside an array.
